...
unique_labels = df['Группа тем'].unique()
label2id = {label: i for i, label in enumerate(unique_labels)}
id2label = {i: label for i, label in enumerate(unique_labels)}
df['Target'] = df['Группа тем'].map(label2id)
df.head(2)

tokenizer = AutoTokenizer.from_pretrained("cointegrated/rubert-tiny2")
rubert_model = AutoModelForSequenceClassification.from_pretrained("cointegrated/rubert-tiny2", num_labels=26, id2label=id2label, label2id=label2id)
...
model_path = "fine-tune-test"
rubert_model.save_pretrained("/t5small", from_pt=True)
